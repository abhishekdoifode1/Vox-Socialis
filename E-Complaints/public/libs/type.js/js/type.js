...
var stylefill = {
  ...
  loadStyles: function (count) {
    ...
    if (sheet) {
      if (sheet.innerHTML) {
        ...
        !stylefill.sheets[sheet.href] &&
        stylefill.options.externalCSS
      ) {
  ...
  getStyleSheets: function (property, func) {
    var sheetstext = new Array(),
      stylesheets = Array.prototype.slice.call(
        document.querySelectorAll('link[href*=".css"]')
      ), // grab stylesheet links - not used yet
      styleEles = document.getElementsByTagName("style");

    if (styleEles.length > 0) {
      for (i in styleEles) {
        if (styleEles[i].innerHTML)
          stylesheets.push(Array.prototype.slice.call(styleEles)[i]); // add on page CSS
      }
    }
  ...
  checkRule: function (property) {
    var propertyCamel = property.replace(/(^|-)([a-z])/g, function (
      m1,
      m2,
      m3
    ) {
  ...
  findRules: function (property, sheettext, func) {
    ...
    if (sheettext) {
      if (!this.checkRule(property)) {
        // check if rule is valid now

        var selreg = new RegExp(
            "([^}{]+){([^}]+)?" +
              property.replace("-", "\\-") +
              "[\\s\\t]*:[\\s\\t]*([^;]+)",
            "gi"
  ...
  runSheets: function () {
    for (var sheet in this.sheetTexts) {
      for (var property in this.properties) {
        if (
          this.sheetTexts.hasOwnProperty(sheet) &&
...
var type = {
      ...
      while (pcount-- > 0) {
        ...
        while (ccount-- > 0) {
          var char = nodechars[ccount],
            ...
            nextcharreg = new RegExp(
              "^(<span[^>]+>)*" + chars[2] + "(</s*spans*>)*$",
              "i"
            );

          if (char === chars[1] && nextchar && nextchar.match(nextcharreg)) {
            nodechars[ccount] =
              '<span style="letter-spacing:' + amount + '">' + char + "</span>";
          }
        }
      }
    ...
    for (var i in rules) {
      ...
      while (elescount-- > 0) {
        ...
        if (rule.value === "emphasis" || rule.value === "all") {
          // emphasized text
          elehtml = elehtml.replace(emphasis, function (
            contents,
            p1,
            p2,
            p3,
  ...
  getStyle: function (ele, style, unit) {
    ...
    if (ele.currentStyle) {
      ret =
        ele.currentStyle[
          style.replace(/-([A-z])/gi, function (a, b) {
